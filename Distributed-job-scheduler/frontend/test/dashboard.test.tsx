...
describe("dashboard boundary pages", () => {
  ...
  it("renders scheduled data and empty state", async () => { vi.mocked(apiClient.scheduledJobs).mockResolvedValue({ data: [{ id: "s1", queueId: "q1", jobType: "digest", payload: {}, cronExpression: "0 * * * *", nextRunAt: "2026-08-23T12:00:00Z", enabled: true, queue: { id: "q1", name: "email", projectId: "p1" } }], pagination: { page: 1, limit: 100, hasMore: false } }); render(<ScheduledPage />); expect(await screen.findByText("digest")).toBeInTheDocument(); });
  it("renders execution data and empty state", async () => { vi.mocked(apiClient.executionsList).mockResolvedValue({ data: [], pagination: { page: 1, limit: 100, hasMore: false } }); render(<ExecutionsPage />); expect(await screen.findByText("No executions found.")).toBeInTheDocument(); });
