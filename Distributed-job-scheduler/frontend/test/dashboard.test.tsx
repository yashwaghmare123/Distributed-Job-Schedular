...
const replace = vi.fn();
vi.mock("next/navigation", () => ({ useRouter: () => ({ replace, push: vi.fn() }), useParams: () => ({ id: "job-1" }), useSearchParams: () => new URLSearchParams() }));
vi.mock("@/lib/api", () => ({ apiClient: { scheduledJobs: vi.fn(), executionsList: vi.fn(), login: vi.fn(), register: vi.fn(), allProjects: vi.fn(), allExecutions: vi.fn() }, setAccessToken: vi.fn() }));
vi.mock("@/lib/socket", () => ({ connectSocket: vi.fn(), disconnectSocket: vi.fn(), resetSocketSubscriptions: vi.fn(), subscribeSocket: vi.fn(() => () => undefined), subscribeSocketStatus: vi.fn((listener: (status: "DISCONNECTED") => void) => { listener("DISCONNECTED"); return () => undefined; }), getRecentSocketEvents: vi.fn(() => []), isSocketConnected: vi.fn(() => false) }));

const { apiClient } = await import("@/lib/api");
function ProjectFixture({ children }: { children: React.ReactNode }) {
  const { setSelectedProjectId } = useSelectedProject();
  useEffect(() => setSelectedProjectId("p1"), [setSelectedProjectId]);
  return <>{children}</>;
}

const render = (ui: React.ReactElement) => rtlRender(<ProjectProvider><ProjectFixture>{ui}</ProjectFixture></ProjectProvider>);

describe("dashboard boundary pages", () => {
  beforeEach(() => { vi.clearAllMocks(); sessionStorage.clear(); vi.mocked(apiClient.allProjects).mockResolvedValue([{ id: "p1", organizationId: "org-a", name: "Project A", description: "", createdAt: "2026-01-01", updatedAt: "2026-01-01" }]); vi.mocked(apiClient.allExecutions).mockResolvedValue([]); });
