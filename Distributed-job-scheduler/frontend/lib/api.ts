import type { DlqEntry, Execution, ExecutionRow, Job, Project, Queue, RetryPolicy, ScheduledJob, Worker } from "./types";
...
export const apiClient = {
  ...
  jobs: (query = "") => api<PageResponse<Job>>(`/jobs${query}`),
  allJobs: (query = "") => fetchAllPages((pageQuery) => api<PageResponse<Job>>(`/jobs${pageQuery}`), query),
  scheduledJobs: (query = "") => api<PageResponse<ScheduledJob>>(`/scheduled-jobs${query}`),
  allScheduledJobs: (query = "") => fetchAllPages((pageQuery) => api<PageResponse<ScheduledJob>>(`/scheduled-jobs${pageQuery}`), query),
  executionsList: (query = "") => api<PageResponse<ExecutionRow>>(`/executions${query}`),
  allExecutions: (query = "") => fetchAllPages((pageQuery) => api<PageResponse<ExecutionRow>>(`/executions${pageQuery}`), query),
  ...
  createBatch: (queueId: string, jobs: unknown[]) => api<Record<string, unknown>>(`/queues/${queueId}/jobs/batch`, { method: "POST", body: JSON.stringify({ jobs }) }),
  ...
  dlq: (query = "") => api<PageResponse<DlqEntry>>(`/dlq${query}`),
