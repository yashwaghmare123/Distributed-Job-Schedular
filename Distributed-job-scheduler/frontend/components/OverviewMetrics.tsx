...
export function OverviewMetrics({ jobs, events, projectId, refreshSignal = 0 }: Props) {
  ...
  return <>
    ...
    <div className="grid overview-kpis"><div className="stat"><span className="stat-label">Total jobs</span><strong className="stat-value">{jobs.length.toLocaleString()}</strong></div><div className="stat"><span className="stat-label">Completed Executions</span><strong className="stat-value">{totalCompleted.toLocaleString()}</strong></div><div className="stat"><span className="stat-label">Failed</span><strong className="stat-value">{totalFailed.toLocaleString()}</strong></div><div className="stat"><span className="stat-label">Running</span><strong className="stat-value">{jobs.filter((job) => job.status === "RUNNING").length.toLocaleString()}</strong></div><div className="stat"><span className="stat-label">Queued</span><strong className="stat-value">{jobs.filter((job) => job.status === "QUEUED").length.toLocaleString()}</strong></div></div>
